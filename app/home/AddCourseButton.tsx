...
export default function AddCourseButton() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Temporarily disabled saving to Firestore
      /*
      ...
      */
      ...
      const allTitles = new Set(aiSyllabus.map((item) => item.title));
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative">
            <h3 className="text-2xl font-semibold text-black mb-4 text-center">AI-Generated Syllabus</h3>
            ...
            <ul className="space-y-2 max-h-80 overflow-y-auto pr-2">
              {syllabus.map((item, index) => (
                <li key={index} className="flex items-start space-x-2">
                  <input
                    type="checkbox"
                    checked={selectedTopics.has(item.title)}
                    onChange={() => {
                      setSelectedTopics((prev) => {
                        const newSet = new Set(prev);
                        newSet.has(item.title)
